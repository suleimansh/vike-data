...
// that an ejected page actually renders + writes lives in examples/vike-view (pages/posts-ejected).
...
import { defineView, crudBlocks, column, field } from '../index.js'
...
test('ejectView: the data hook inlines the view descriptor + route', () => {
  ...
  assert.match(data, /const view = defineView\(/)
  ...
  assert.match(data, /from 'vike-view'/)
...
test('ejectView: a view with no scope emits `const scope = undefined`', () => {
  const view = defineView({ route: '/notes', sections: crudBlocks({ table: 'notes' }) })
...
test('ejectView (vue): emits a .vue SFC page', () => {
  ...
  assert.match(page, /from 'vike-view\/vue'/)
...
test('ejectView (react): the page renders sections through <Blocks>', () => {
  ...
  assert.match(page, /from 'vike-view\/react'/)
