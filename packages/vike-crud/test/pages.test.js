...
import { definePage, crudBlocks, resolveViewTables } from '../index.js'
...
const postsView = definePage({ route: '/posts', sections: crudBlocks({ table: 'posts' }) })
const dashView = definePage({ route: '/dash', sections: [{ block: 'markdown', source: '# Hi' }] })
...
test('viewPages builds one page per view route, sharing the generic Page + data hook', () => {
  ...
  assert.equal(pages[0].Page, 'import:vike-crud/react/ViewPage:default')
  assert.equal(pages[0].data, 'import:vike-crud/react/viewData:viewData')
})

test('viewPages skips a view with no route, and flattens cumulative contributions', () => {
  const routeless = definePage({ sections: [{ block: 'markdown', source: 'x' }] })
