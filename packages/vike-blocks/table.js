...
// a client-side sort toggle (local UI state, like the code block's copy button — no actions layer).
// This is display-only; the interactive non-schema FORM block waits on the actions axis (#385).
...
export const table = defineBlock('table', {
  ...
  // keys) + the rows passed through. All plain data, so both renderers stay thin.
  resolve({ props }) {
