...
// is a single <svg> with a fixed viewBox that scales UNIFORMLY to its container (CSS width:100%,
// height:auto): responsive with no measuring, so SSR and the client render identically (no width jump /
// flicker), and because the scale is uniform nothing distorts — circles stay round and bar corners
// stay square. `height` sets the viewBox aspect (the rendered height scales with the container width).
...
export function ChartView({ data = [], type = 'bar', height = 160, color = DEFAULT_CHART_COLOR, max }) {
  ...
  const geo = chartGeometry(points, { width: CHART_WIDTH, height, max })
  const hasLabels = points.some((p) => p.label !== '')

  return (
    <div style={chartRootStyle(height)} data-slot="chart">
      <svg viewBox={`0 0 ${CHART_WIDTH} ${height}`} role="img" style={chartSvgStyle()}>
        <line x1="0" y1={geo.baselineY} x2={CHART_WIDTH} y2={geo.baselineY} style={chartBaselineStyle()} />
