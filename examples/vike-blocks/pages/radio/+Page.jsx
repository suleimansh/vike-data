...
import { definePage, resolvePage, radioGroup, field } from 'vike-blocks'
  ...
  return (
    <div style={{ maxWidth: 480, margin: '2rem auto', padding: '0 1rem', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <p style={{ color: '#64748b', lineHeight: 1.6 }}>
        A dep-free, theme-native radio group with an animated selection. <code>radioGroup().option('pro', 'Pro').value('pro')</code> —
      ...
      <Section label="Pick a plan">
        {Show([radioGroup().option('free', 'Free').option('pro', 'Pro').option('team', 'Team').value('pro')])}
      </Section>

      <Section label="Disabled group">
        {Show([radioGroup().option('a', 'Option A').option('b', 'Option B').value('a').disabled()])}
      </Section>
      ...
      <Section label="Inside a field">
        {Show([
          field('Notifications')
            ...
            .control(radioGroup().option('all', 'Every update').option('daily', 'Daily digest').option('none', 'Never').value('daily')),
        ])}
      </Section>
