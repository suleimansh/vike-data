...
const catalog = [
  ...
  { name: 'Context-menu', href: '/context-menu', tag: 'interactive', desc: 'A right-click menu anchored at the cursor — the last of the menu family. Wrap a region with .on(block); the menu portals to <body>, flips at the viewport edge, and reuses the dropdown item chrome + roving arrow-key focus. Non-modal; closes on outside-click / Escape / scroll.' },
  { name: 'Stepper', href: '/stepper', tag: 'interactive', desc: 'A multi-step wizard — the multi-step-form surface. A numbered progress header (complete / current / upcoming + connectors) over one step’s content, Back / Next navigation, and steps that compose any blocks (fields, a form). The sequential sibling of tabs; step state seeded so SSR renders without a flash.' },
  { name: 'Description-list', href: '/description-list', tag: 'container', desc: 'A key-value / metadata grid (Ant Descriptions) — the record-detail, summary, and metadata surface. Term/value pairs in a responsive grid; a value is a string or nested blocks (a badge, a link). columns / bordered / per-item span. Static, semantic <dl>, theme-native.' },
  { name: 'Rating', href: '/rating', tag: 'form', desc: 'A star-rating control — hover to preview, click to set, arrow keys to adjust. Half-stars, a read-only display mode (product averages), and a hidden input for native submit. Pairs with field. Dep-free, theme-native.' },
