...
- **Owner-guarded actions in a table** (`/actions-demo`): the user's posts as a `table` block with a **row-action column** — per row, `button('Publish').action('publish').params({ id: '$row.id' })` and a Delete button. The table renderer resolves `$row.id` against each row; the click POSTs `/_actions/<name>`. The actions (`pages/actions.js`) are guarded (`'authed'`), enforce ownership in their write filter, and return an `onSuccess` toast naming the post. `+Wrapper.jsx` mounts `<ActionsProvider>` + `<Toaster>`, so the click fires the toast and reloads. Wired with `vike-actions`; the endpoint is one middleware pointed at `pages/actions.js`.
